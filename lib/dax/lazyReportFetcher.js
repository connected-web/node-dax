var fs = require('fs');
var makeDaxRequest = require('./reportFetcher');
var logger = require('../util/logger');

function fetchReport(name, reportRequest, storagePath, requester) {

    name = name || false;
    reportRequest = reportRequest || false;
    storagePath = storagePath || __dirname + '/../../daxCache/';

    var cachedFile = storagePath + '/' + name + '.json';

    return new Promise(function(accept, reject) {
        if (reportRequest) {
            // DAX is restricted to 30 requests per minute
            // Since the data is daily, we should not be making more than 1 request a day per URL
            // Check for local cached file:

            // if(cachedFile is older than 1 day, then go get a new one)
            try {
                var buffer = fs.readFileSync(cachedFile, 'utf-8');
                var jsonData = JSON.parse(buffer);
                accept(jsonData);
            } catch (ex) {
                reject(ex);
            }
        } else {
            if (name) {
                reject('No report with the name ' + name + ' has been defined.');
            } else {
                reject('No name set to save report');
            }
        };

        // Later, go do a file update check
        process.nextTick(function() {
            logger.log('Go do file update');
            checkFileAge(cachedFile).then(function() {
                return makeDaxRequest(name, reportRequest, storagePath, requester);
            }).catch(function(err) {
                logger.error('Cached check failed:', err);
            });
        });
    });

    function checkFileAge(filePath) {
        // 12 hours, plus 0-1 hours, so that not all reports are refreshed at the same time
        var MAX_AGE_IN_MS = ((3600 * 12) + Math.round(Math.random() * 3600)) * 1000;

        logger.log('Cached file check', filePath);
        return new Promise(function(accept, reject) {
            fs.stat(filePath, function(err, stat) {
                var endTime, now;
                if (err) {
                    logger.log('Check file age failed: ', err);
                    return accept(true);
                }
                now = new Date().getTime();
                endTime = stat.mtime.getTime() + MAX_AGE_IN_MS;
                logger.log('Check file age: ', MAX_AGE_IN_MS, ' ms')
                logger.log('  File MTIME : ', stat.mtime.getTime(), stat.mtime);
                logger.log('  End time   : ', endTime, new Date(endTime));
                logger.log('  Date now   : ', now, new Date(now));
                if (now > endTime) {
                    logger.log('File needs updating');
                    accept(true);
                } else {
                    logger.log('File does not need updating');
                    reject(false);
                }
            });
        });
    }
}

module.exports = fetchReport;