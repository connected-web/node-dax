...
function fetchReport(name, reportRequest, storagePath) {
	...
	if (reportRequest) {
		...
		try {
			fs.readFile(cachedFile, 'utf-8', processCachedFile);
		} catch (ex) {
			res.jsonp({
				error: true,
				message: ex
		...
		if (name) {
			res.jsonp({
				error: true,
				message: 'No report with the name ' + name + ' has been defined.'
			});
		} else {
			res.jsonp({
				route: instance.routes[0],
				availableReports: reportRequestObjects
			});
		}
	};

	function processCachedFile(error, contents) {
		if (error) {
			// Error accessing cached file, make a request
			res.jsonp({
				error: true,
				message: error
			});
